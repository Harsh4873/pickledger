"""Algorithmic FIFA World Cup moneyline, spread, and totals model.
...
def _feedback_profile(date_iso: str) -> dict[str, Any]:
    ...
    for path in sorted(MODEL_CACHE_DIR.glob("20*.json")):
        ...
        try:
            ...
            continue
        bucket = (payload.get("models") or {}).get("fifa_world_cup") if isinstance(payload, dict) else {}
...
def _matchup_picks(
    ...
    raw_home_xg = expected_goals(home, away, venue_multiplier)
    raw_away_xg = expected_goals(away, home, venue_multiplier)
