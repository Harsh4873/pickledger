...
FRESH_STAMP = "2026-09-22T20:00:00Z"  # 4 hours before tip
STALE_STAMP = "2026-09-21T22:00:00Z"  # 26 hours before tip
...
def test_stale_or_untimestamped_wnba_prop_does_not_ship():
    import player_props.variants as variants

    fresh = _wnba_prop(stamp=FRESH_STAMP, odds=-120, player="Breanna Stewart")
    stale = _wnba_prop(stamp=STALE_STAMP, odds=-137, player="Olivia Miles")
    missing = _wnba_prop(stamp=None, odds=-115, player="A'ja Wilson")

    assert fresh_observed_book_price(fresh) is True
    assert fresh_observed_book_price(stale) is False
    assert fresh_observed_book_price(missing) is False
    # 26 hours before tip is the reported WNBA prop failure.
    assert (parse_timestamp(START) - parse_timestamp(STALE_STAMP)).total_seconds() / 3600 == 26

    selected = variants._select_variant([stale, missing, fresh], "season")
    ...
    assert "Olivia Miles" not in {pick["player_name"] for pick in selected}
